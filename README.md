...
|---|---|---|
| Throughput | 1.94 FPS | 2.83 FPS |
| Lane fit valid | 59% | 69% |
| Mean \|steering\| | 4.4° | 2.5° |
| FSM | FOLLOW 60%, CHANGING 22% | LANE_KEEP 91%, FOLLOW 9% |
...
|---|---|---|
| `requirements-cpu.txt` | no NVIDIA GPU — your laptop | `onnxruntime` |
| `requirements-gpu.txt` | CUDA GPU — Colab | `onnxruntime-gpu` |

Both files are self-contained; they are identical apart from which ONNX Runtime
wheel they pull. Install exactly one.
...
```
data/input/footage2.mp4     US rural road, 55.6 s, 862x484, 30 fps, 1668 frames
```
...
```bash
# the bundled demo clip -- 30 fps source, so stride 2 gives the 15 fps output
...
| `--stride N` | process every Nth frame. Use 2 on 30 fps footage, 4 on 60 fps |
...
python run.py --video data/input/clip.mp4 --stride 2              # 30fps source
```
